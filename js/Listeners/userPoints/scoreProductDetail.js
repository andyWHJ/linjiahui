...
function detail(id) {
	...
	var checkStr = "9C958EFC054CA3B9796DEFF28EBD6016";
	var loginUserId = 588;
	var isMember = 1;
...
function detailpage() {
	...
	var id = getLocalValue("scoreExchangeId");//add
    id=2;
	var returnData = str2obj(window.localStorage.getItem("scoredetail_"+id));
	...
	for (var i = 0; i < PictureUrls.length; i++) {
		...
		if (window.localStorage.getItem(PictureUrls[i]) != undefined && window.localStorage.getItem(PictureUrls[i]) != 'undefined') {
				...
				var imageBase64 = obj;
				imgHTML += '<img onclick="showAdWithTitle(\'' + PictureUrls[i] + '\',\'' + '商品详情' + '\')" src="data:image/png;base64,' + imageBase64 + '" height="'+ 0.6*width +'" width="'+ 0.6*width +'" style="line-height:100px;margin-top:12px;"/>';
		} else {
			...
			imgHTML += '<img onclick="showAdWithTitle(\'' + PictureUrls[i] + '\',\'' + '商品详情' + '\')"  src="' + PictureUrls[i] + '" height="'+ 0.6*width +'" width="'+ 0.6*width +'" style="line-height:100px;margin-top:12px;"/>';
		}
...
var scoreId = window.localStorage.getItem("scoreExchangeId");
...
function userCareScoreProduct(productId) {
	...
	//alert(checkStr+"-"+loginUserId+"-"+isMember)
