if (debug) {
	...
	var promotionDetail = getLocalValue("promotionDetail");
	var promotionId = getLocalValue("promotionId");
	var promotionTitle = getLocalValue("promotionTitle");
	var shareMessage = getLocalValue("shareMessage");
}
//add
...
function iconShow(name) {
	...
	var left = scrollleft - 55 + (allWidth / 2);
...
function refreshPromotionFavorStatus(promotionId) {
	...
	var isMember = getLocalUserValue("isMember");
	if (checkNetWorkState()) {
	ajaxRequest(loginUserId +"_"+promotionId+"_promotionFavorStatus", promotionFavorUrl, "&promotionId=" + promotionId+"&loginUserId=" + loginUserId + "&checkStr=" + checkStr +"&isMember=" + isMember, null, promotionSuccess, promotionError, false);
	} else {
		if(window.localStorage.getItem(loginUserId +"_"+promotionId+"_promotionFavorStatus") == null){
			goError();
		}else{
			promotionSuccess();
		}
...
function promotionSuccess() {
	var loginUserId = getLocalUserValue("loginUserId");
	var promotionId = getLocalValue("promotionId");
	var returnData = str2obj(window.localStorage.getItem(loginUserId +"_"+promotionId+"_promotionFavorStatus"));
