...
$(function() {
	var V = function() {
		addWare($("#currentWareId").val(), 1, false, $("#resourceType").val(), $("#resourceValue").val(), $("#sid").val())
	};
	var N = function() {
	...
	function y(ac, ad, ab) {
		...
		Z.onerror = function() {
			G.stop();
			$("#spinner").hide();
			var ae = O(HTML5_DEFAULT_IMG_180x180);
			ad(ae)
		}
	}
	...
	function o(Z) {
		...
		if (Z && Z.ware && Z.ware.images && Z.ware.images.length > 0) {
			...
			$.each(Z.ware.images, function(ac, ab) {
				if (ab && ab.newpath) {
					if (ac == 0) {
						d = 0;
						y(ab.newpath.replace("/n4/", "/n1/"), A, 0)
					}
					aa = aa + ab.newpath.replace("/n4/", "/n1/") + ","
				}
			});
			$("#imgs").val(aa);
	...
	function b(aa) {
		...
		if (aa && aa.proColors && aa.proColors.length > 0) {
			$("#color_parent").show();
			$("#color").empty();
			$.each(aa.proColors, function(ac, ab) {
		...
		if (aa && aa.proSizes && aa.proSizes.length > 0) {
			$("#size_parent").show();
			$("#size").empty();
			$.each(aa.proSizes, function(ac, ab) {
				if (aa.ware.wareId == ab.wareId) {
		...
		if (aa && aa.stock) {
			if (!aa.useJdPrice) {
				$("#jd_price").attr("src", "http://price.360buyimg.com/gp" + aa.stock.wareId + ",3.png")
			} else {
				var Z = "";
			...
			if (!aa.useJdPrice) {
				$("#jd_price").attr("src", "http://price.360buyimg.com/gp" + aa.ware.wareId + ",3.png")
			} else {
				var Z = "";
	...
	function K(Z) {
		if (Z && Z.stock) {
			...
			$.each(Z.provinceList, function(ab, aa) {
			...
			if (Z.stock.cityId) {
				...
				$("#store").show();
				$.each(Z.cityList, function(ab, aa) {
